...
extension NameCardViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func jumpToTags() {
        let vc = NameCardTagsViewController()
    ...
    func jumpToIntro() {
        let vc = IntroSettingViewController()
        self.navigationController?.pushViewController(vc, animated: true)
    }
...
extension NameCardViewController {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingImage image: UIImage, editingInfo: [String: AnyObject]?) {
        ...
        if imagePick == 0 {
            ...
            coverImage.faeUploadCoverImageInBackground { (code: Int, _: Any?) in
                //                print(code)
                //                print(message)
                if code / 100 == 2 {
                    //                self.imageViewAvatarMore.image = image
    ...
    func showPhotoSelected(_ sender: Int) {
        ...
        let showLibrary = UIAlertAction(title: "Choose from library", style: .default) { (_: UIAlertAction) in
            self.imagePicker.sourceType = .photoLibrary
            menu.removeFromParentViewController()
            self.present(self.imagePicker, animated: true, completion: nil)
        }
        let showCamera = UIAlertAction(title: "Take photos", style: .default) { (_: UIAlertAction) in
