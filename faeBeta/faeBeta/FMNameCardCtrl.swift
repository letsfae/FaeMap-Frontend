        ...
        if let _ = realm.filterUser(id: "\(id)") {
            ...
            getFromURL("users/\(id)/name_card", parameter: nil, authentication: headerAuthentication()) { status, result in
                if status / 100 == 2 && result != nil {
                    ...
                    let newUser = RealmUser(value: ["\(Key.shared.user_id)_\(id)", String(Key.shared.user_id), "\(id)", profileJSON["user_name"].stringValue, profileJSON["user_name"].stringValue, NO_RELATION, "", ""])
                    try! realm.write {
