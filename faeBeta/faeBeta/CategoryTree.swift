    ...
    private func parseCategoriesCSV() {
        ...
        do {
            ...
            for rowIdx in 0..<rows.count {
                ...
                let icon_2 = cols[3].trimmingCharacters(in: .whitespaces)
                let icon_3 = cols[5].trimmingCharacters(in: .whitespaces)
                let icon_4 = cols[7].trimmingCharacters(in: .whitespaces)
...
//                let sub2 = class2_to_3[key]!.subtracting(val)
        ...
        for (key, val) in class34 {
            if class3_to_4[key] == nil {
                print("class34key \(key)")
            } else {
//                let sub1 = val.subtracting(class3_to_4[key]!)
//                let sub2 = class3_to_4[key]!.subtracting(val)
                let diff = val.symmetricDifference(class3_to_4[key]!)
                if !diff.isEmpty {
                    print("diff34 \(diff)")
                }
                //                    print("class34 \(key) \(sub1) \(sub2)")
            }
        }
        
        for (key, _) in class4_to_5 {
            if !class4.contains(key) {
                print("diff45 \(key)")
            }
        }
        
        for c4 in class4 {
            if class4_to_5[c4] == nil {
                print("diff45.. \(c4)")
            }
        }
        
        // test for category_to_id dictionary
        for (name, icon) in categories {
            if category_to_icon[name] == nil {
                print("category_to_iconName1 \(name)")
            } else if category_to_icon[name]! != "\(icon)" {
                print("category_to_iconId1 \(name) \(icon)")
            }
        }
        for (name, icon) in category_to_icon {
            if categories[name] == nil {
                print("category_to_iconName2 \(name)")
            } else if "\(categories[name]!)" != icon {
                print("category_to_iconId2 \(name) \(icon)")
            }
        }
    }
