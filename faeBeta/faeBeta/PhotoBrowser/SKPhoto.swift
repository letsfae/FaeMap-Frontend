...
open class SKPhoto: NSObject, SKPhotoProtocol {
    ...
    convenience init(message: RealmMessage) {
        self.init()
    ...
    open func loadUnderlyingImageAndNotify() {
        if let message = realmMessage {
            guard ["[Picture]", "[Gif]"].contains(message.type) else { return }
...
extension SKPhoto {
    ...
    static func photoWithRealmMessage(_ message: RealmMessage) -> SKPhoto {
        return SKPhoto(message: message)
    }
}
